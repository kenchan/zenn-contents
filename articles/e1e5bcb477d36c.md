...
たとえば、Railsを立ち上げる場合であれば、`--bind 0.0.0.0`といったオプションが必要になります。ただし、私の環境ではdockerを使っている場合はデフォルトで`0.0.0.0`にバインドされるようで、あまり困ることはありませんでした。WindowsからWSL内へのアクセスができないときは、この点を確認してください。
