...
detached起動は、最初`trap '' HUP`なしで実装していました。ところが実際に動かしてみると、workerフェーズは呼び出されたはずなのに、workspaceの作成やagentの起動が何も起きません。ログにも実行の痕跡が残らず、まるで呼び出し自体がなかったかのように見えました。

ログを仕込んだり別のコマンドでためすなど検証をしていくと、次のようなことが起きていることがわかりました。

1. popupフェーズのbashが`exit 0`する
2. herdrがpopupのpaneを破棄し、PTY(疑似端末)を閉じる
3. カーネルが、そのPTYにつながっていたプロセスへSIGHUPを送る
4. まだ`setsid`を呼ぶ前のworkerも、この「つながっていたプロセス」に含まれている
5. workerはSIGHUPに対して何もハンドラを設定していないので、デフォルトの動作である終了をする

`&`で起動した子プロセスは、`setsid`を実行して初めて元の端末から切り離されます。bashが子プロセスをfork()してから、その子が実際に`setsid`を呼ぶまでにはわずかな時間差があります。その隙間でPTYが破棄されると、子はまだ元の端末とつながったままSIGHUPを受け取ってしまいます。

修正は、spawnの直前に置いた`trap '' HUP`です。「SIGHUPを無視する」という設定は、fork()やexecを挟んでも消えません(execでリセットされるのはハンドラの中身で、無視するという指定だけは特別に引き継がれます)。これで`setsid`を呼ぶ前にSIGHUPが届いても、workerは死なずに済みます。`setsid`を呼んだ後は制御端末を持たない新しいセッションに移るので、そもそもPTYの破棄自体が届かなくなります。
...
| リポジトリ | star | 実装 |
| --- | --- | --- |
| [openclaw/crabbox](https://github.com/openclaw/crabbox) | 1250 | sh 124行 + Go CLIへの委譲 |
| [madarco/agentbox](https://github.com/madarco/agentbox) | 333 | TypeScript/Node、herdr層760行、ソケット+CLI |
| [smarzban/herdr-file-viewer](https://github.com/smarzban/herdr-file-viewer) | 327 | Rust、29,000行のTUI |
| [persiyanov/herdr-reviewr](https://github.com/persiyanov/herdr-reviewr) | 313 | Rust、22,000行のTUI |
| [ogulcancelik/herdr-browser](https://github.com/ogulcancelik/herdr-browser) | 245 | TypeScript/Bun直実行、依存ゼロ、ビルドなし |
| [AltanS/collie](https://github.com/AltanS/collie) | 227 | TypeScript/Bun 6,300行 + sh 828行、.env設定 |
| [cloudmanic/herdr-plus](https://github.com/cloudmanic/herdr-plus) | 194 | Go 3,700行のTUI、1アイテム1 TOML設定 |
| [dcolinmorgan/herdr-remote](https://github.com/dcolinmorgan/herdr-remote) | 172 | Python、フックは20行 |
| [nikok6/herdr-mirror](https://github.com/nikok6/herdr-mirror) | 88 | Rust 8,500行、ソケット直結 |
| [edmundmiller/dotfiles](https://github.com/edmundmiller/dotfiles) | 77 | Python標準ライブラリ、66〜282行のプラグインを3つ、完全ゼロコンフィグ |
| [crafts69guy/herdr-ghq](https://github.com/crafts69guy/herdr-ghq) | (類似プラグイン) | Rust 9,100行 + bash 1,100行、fzfを捨てて自前のratatui TUI |
