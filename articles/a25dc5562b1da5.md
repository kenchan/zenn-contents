...
「設定」→「開発者」→「設定を編集」をクリックしたとき、インストール時期によってはエラーダイアログが表示されます。

![「場所が利用できません」エラーダイアログ](/images/a25dc5562b1da5/01.webp)

通常の設定ファイルのパスは `%APPDATA%\Claude\claude_desktop_config.json` です。エラーが出た場合は、エクスプローラーのアドレスバーに `%LOCALAPPDATA%\Packages` と入力し、`Claude_` から始まるフォルダを探してください。その中の `LocalCache\Roaming\Claude\claude_desktop_config.json` が設定ファイルです。
